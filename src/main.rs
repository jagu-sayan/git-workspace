...
struct Args {
    ...
    workspace: PathBuf,
    #[arg(short = 'o', long = "output", value_enum, default_value = "simple")]
    output: OutputFormat,
    #[command(subcommand)]
    command: Command,
}
...
fn handle_main(args: Args) -> anyhow::Result<()> {
    ...
    match args.command {
        Command::List { full } => list(&workspace_path, full, output)?,
        Command::Update { threads } => {
            lock(&workspace_path, output)?;
            update(&workspace_path, threads, output)?
        }
        Command::Lock {} => {
            lock(&workspace_path, output)?;
        }
        Command::Archive { force } => archive(&workspace_path, force, output)?,
        Command::Fetch { threads } => fetch(&workspace_path, threads, output)?,
        Command::Add { file, command } => add_provider_to_config(&workspace_path, command, &file)?,
        Command::Run {
            threads,
            command,
            args,
        } => execute_cmd(&workspace_path, threads, output, command, args)?,
        Command::SwitchAndPull { threads } => {
            pull_all_repositories(&workspace_path, threads, output)?
        }
    };
    Ok(())
}
